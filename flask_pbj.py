...
def copy_dict_to_pb(instance, dictionary):
    ...
    for key, value in dictionary.items():
        ...
        # If the value is iterable, copy the list into the repeated field
            ...
            # dunno, quick hack to get this shit working (string doesn't have `extend` method)
...
class ProtobufCodec(object):
    ...
    def make_response(self, data, status_code, headers):
        ...
        if (status_code % 100 == 4 or status_code % 100 == 5) and self.error_type:
...
class api(object):
    ...
    def response_mimetype(self, _request):
        ...
        res = _request.accept_mimetypes.best_match(
            self.mimetypes
        )
        # if the client doesn't specify content return types (like in the case of FlaskClient
        # that is used for testing...) just use json as the default
        return "application/json" if res is None else res
